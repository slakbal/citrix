...
abstract class CitrixAbstract
{
	...
	public function sendRequest()
	{
		...
		if ( !$this->http_client instanceof HttpClient ) {
			...
			$this->http_client = new HttpClient( [ 'base_uri' => $this->base_uri,
			                                       'port'     => $this->port,
		...
		try {
			...
			switch ( $this->getHttpMethod() ) {
				...
				case 'GET':
					$response = $this->http_client->get( $this->getUrl(), [ 'headers' => [ 'Content-Type'  => 'application/json; charset=utf-8',
				...
				case 'POST':
					$response = $this->http_client->post( $this->getUrl(), [ 'headers' => [ 'Content-Type'  => 'application/json; charset=utf-8',
					                                                                        'Accept'        => 'application/json',
					                                                                        'Authorization' => 'OAuth oauth_token=' . $this->getAccessToken() ],
					                                                         'json'    => $this->getParams() ] );
					break;
				...
				case 'DELETE':
					$response = $this->http_client->delete( $this->getUrl(), [ 'headers' => [ 'Content-Type'  => 'application/json; charset=utf-8',
					                                                                          'Accept'        => 'application/json',
					                                                                          'Authorization' => 'OAuth oauth_token=' . $this->getAccessToken() ],
					                                                           'json'    => $this->getParams() ] );
					break;
